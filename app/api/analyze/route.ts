...
async function getPublicInvidiousInstances(): Promise<string[]> {
  try {
    console.log(
      '[INVIDIOUS] Mengambil daftar instance publik yang aktif dari tracker...',
    );
    ...
    console.error(
      '[INVIDIOUS] Gagal mengambil daftar instance dari tracker:',
      error,
    );
    return [];
  }
...
async function tryInvidiousRequest(
  ...
  const res = await fetch(
    `${cleanBaseUri}/api/v1/videos/${videoId}?local=true`,
    {
      headers: { Accept: 'application/json' },
  ...
  console.log(
    `[INVIDIOUS] Menemukan stream video, mulai mengunduh dari: ${downloadUrl}`,
  );
  ...
  console.log(
    `[INVIDIOUS] Selesai mengunduh video via Invidious ke: ${finalPath}`,
  );
  return finalPath;
}
...
async function downloadViaInvidious(
  ...
  if (!videoId) {
    throw new Error(
      'ID video YouTube tidak dapat ditemukan dari tautan tersebut',
    );
  }
  ...
  console.log(
    `[INVIDIOUS] Mulai memproses failover pada ${allInstances.length} instansi Invidious...`,
  );
      ...
      try {
        ...
        if (urlInput && videoId) {
          ...
          try {
            ...
            console.warn(
              `[TRANSCRIPT] Gagal mengambil transkrip YouTube: ${err.message || err}. Mencoba mengunduh video...`,
            );
          }
        }
